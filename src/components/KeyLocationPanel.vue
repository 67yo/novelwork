...
import type { KeyLocation } from "@/lib/spatiotemporal";
import {
  emptyLocation,
...
const props = defineProps<{
  novelId: string;
  name: string;
  data: KeyLocation | null | undefined;
  busy?: boolean;
}>();

const emit = defineEmits<{
  save: [payload: { name: string; keyLocation: KeyLocation; extracted: string }];
}>();

const { t } = useI18n();
const draftName = ref(props.name);
const draft = reactive(normalizeLocation(props.data ?? emptyLocation()));

watch(
  () => [props.name, props.data] as const,
  () => {
    draftName.value = props.name;
    Object.assign(draft, normalizeLocation(props.data ?? emptyLocation()));
  },
);

function commit() {
  const keyLocation = normalizeLocation(draft);
  const title =
    keyLocation.name.trim() ||
    draftName.value.trim() ||
    t("workspace.st.locationUntitled");
  keyLocation.name = keyLocation.name.trim() || title;
  emit("save", {
    name: title,
    keyLocation,
    extracted: formatKeyLocationExtracted(keyLocation),
  });
}

type AiField = keyof KeyLocation | "whole";
...
const aiFieldLabel = computed(() => {
  ...
  if (!f) return "";
  if (f === "whole") return t("workspace.st.aiRewriteWhole");
  if (f === "name") return t("workspace.st.locName");
  if (f === "features") return t("workspace.st.locFeatures");
  if (f === "terrain") return t("workspace.st.locTerrain");
  return t("workspace.st.locFaction");
});
...
async function confirmAi() {
  ...
  aiError.value = "";
  try {
    const ctx = formatKeyLocationExtracted(normalizeLocation(draft));
    if (field === "whole") {
      const instruction = `${note}

请只输出一个 JSON 对象，字段：name、features、terrain、faction（可用中文键 名称/特征/地貌/控制势力）。不要解释、不要 markdown 围栏。`;
      ...
      const parsed = parseKeyLocationJson(out);
      if (!parsed) {
        aiError.value = t("workspace.st.locationParseFail");
        return;
      }
...
<template>
  <div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto text-sm">
    <div class="flex items-start justify-between gap-2">
      <p class="text-[11px] text-muted-foreground">{{ t("workspace.st.locationCardHint") }}</p>
      <Button
        type="button"
        size="sm"
        variant="outline"
      ...
      >
        ...
        {{ t("workspace.st.aiRewriteWhole") }}
      </Button>
    </div>
    <div class="space-y-1">
      <div class="flex items-center justify-between gap-2">
        <label class="text-xs text-muted-foreground">{{ t("workspace.st.locName") }}</label>
        <Button
          type="button"
          size="sm"
          variant="ghost"
      ...
      <Input
        v-model="draft.name"
        class="h-8"
        :placeholder="t('workspace.st.locNamePh')"
        :disabled="busy"
        @change="commit"
      />
    </div>
    <div
      v-for="field in (['features', 'terrain', 'faction'] as const)"
      :key="field"
      class="space-y-1"
    >
      <div class="flex items-center justify-between gap-2">
        <label class="text-xs text-muted-foreground">
          {{
            field === "features"
              ? t("workspace.st.locFeatures")
              : field === "terrain"
                ? t("workspace.st.locTerrain")
                : t("workspace.st.locFaction")
          }}
        </label>
        <Button
          type="button"
    ...
    >
      <div class="w-full max-w-md rounded-lg border bg-background p-5 shadow-lg" role="dialog" aria-modal="true">
        ...
        <Textarea
          ...
          :placeholder="
            pendingField === 'whole'
              ? t('workspace.st.aiWholePromptPh')
