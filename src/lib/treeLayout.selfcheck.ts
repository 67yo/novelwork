...
// 世界观子卡与主卡、子卡之间不重叠
{
  ...
  const nodes = [
    ...
    mk("wv_history_culture", "wv_history_culture", 3),
  ...
  applyAutoLayout(nodes, []);
  const wv = nodes.filter(
    (n) => n.kind === "knowledge" && n.knowledge?.slot?.startsWith("wv_"),
  );
  const overlap = (a: (typeof nodes)[0], b: (typeof nodes)[0]) => {
    const w = 200;
    const h = 88;
    return (
