...
- **抽取时机**：**仅手动**（预生成 / 精修均不自动抽）。命令：`regenerate_chapter_memory(novel_id, node_id, user_notes, memory_node_ids)`；模型：`knowledge_model`。抽取走 rig Extractor 结构化（`plot` / `facts`）；失败或空则回退原 `llm_complete` + 文本切分。模拟模式不写库。  
